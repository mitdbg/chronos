# Transactional Agent Runtime (Chronos)
...
Transactional Agent Runtime (Chronos) is a novel runtime infrastructure that brings database-grade transactional guarantees to autonomous AI agent workflows. As LLM-powered agents evolve from stateless chatbots into long-running, state-modifying systems, they require robust mechanisms to safely explore actions, recover from failures, and maintain data integrity across heterogeneous backends.

Chronos introduces **Virtual Branching**—a Git-like branching model for agent execution—where agents operate in isolated, ephemeral "universes." Successful workflows commit their changes to the main state, while failed or hallucinated workflows are cleanly discarded without side effects.
...
**Analogy:** Just as Git allows developers to code on a branch without breaking `main`, Chronos allows agents to operate in ephemeral, isolated "universes."
...
Chronos wraps the ReAct (Reasoning-Act) loop within a distributed transaction:
...
Drawing from the Epoxy polystore transaction protocol, Chronos implements **zero-copy branching** via multi-version concurrency control (MVCC) using two metadata fields on every record:
...
class SQLDatabaseShim(TMCPShim):
        ...
        for table in self.tracked_tables:
            self.conn.execute(f"""
                ALTER TABLE {table} ADD COLUMN IF NOT EXISTS
                    _chronos_begin_txn BIGINT NOT NULL DEFAULT 0,
                    _chronos_end_txn BIGINT NOT NULL DEFAULT {INFINITY}
            """)
            # Composite index for efficient visibility predicate evaluation
            self.conn.execute(f"""
                CREATE INDEX IF NOT EXISTS idx_chronos_mvcc_{table} 
                ON {table} (_chronos_begin_txn, _chronos_end_txn)
            """)
    ...
    def _visibility_where(self, snapshot: TxnSnapshot) -> str:
        ...
        return (
            f"((_chronos_begin_txn < {xmin}) OR (_chronos_begin_txn IN ({rc})) OR (_chronos_begin_txn = {x})) "
            f"AND ((_chronos_end_txn >= {xmin}) AND (_chronos_end_txn NOT IN ({rc})) AND (_chronos_end_txn != {x}))"
        )
...
**Key insight:** OverlayFS already implements copy-on-write semantics natively in the kernel. Writes go to the `upperdir`; reads fall through to the `lowerdir`. Deletes create whiteout files. This is *exactly* the branching model Chronos needs for filesystems.
...
│       upperdir=/tmp/chronos/<txn_id>/upper,                             │
│       workdir=/tmp/chronos/<txn_id>/work \                              │
│    /tmp/chronos/<txn_id>/merged                                         │
│                                                                     │
│  Behavior:                                                          │
...
├──────────────────────────┤     ├──────────────────────────┤
│                          │     │                          │
│  upper: /tmp/chronos/        │     │  upper: /tmp/chronos/        │
│    txn-abc-123/upper/    │     │    txn-def-456/upper/    │
│                          │     │                          │
│  merged: /tmp/chronos/       │     │  merged: /tmp/chronos/       │
...
```python
# Path structure:
#   /tmp/chronos/<branch_id>/upper/   ← CoW writes
#   /tmp/chronos/<branch_id>/work/    ← OverlayFS workdir
#   /tmp/chronos/<branch_id>/merged/  ← unified POSIX view
#
# For subtransactions, branch_id includes parent lineage:
#   /tmp/chronos/<parent_txn_id>:<sub_txn_id>/upper/
#   /tmp/chronos/<parent_txn_id>:<sub_txn_id>/merged/
#
# This makes it trivial to:
#   - List all active branches:  ls /tmp/chronos/
#   - Find subtransactions:      ls /tmp/chronos/ | grep "^parent_id:"
#   - Clean up after abort:      umount + rm -rf /tmp/chronos/<branch_id>/
```
...
class OverlayFSShim(TMCPShim):
    ...
    Each transaction gets its own OverlayFS mount:
      ...
      - upperdir = /tmp/chronos/<branch_id>/upper (branch-specific writes)
      - merged   = /tmp/chronos/<branch_id>/merged (unified view)
    
    Unix tools (grep, gcc, pytest, git diff, etc.) work unmodified
    on the merged directory.
    """
    
    CHRONOS_ROOT = Path("/tmp/chronos")
    ...
    def _branch_dir(self, branch_id: str) -> Path:
        ...
        return self.CHRONOS_ROOT / branch_id
...
class VectorStoreShim(TMCPShim):
        ...
        for v in vectors:
            ...
            if existing:
                ...
                self.client.update_metadata(
                    id=v.id,
                    filter=self._visibility_filter(txn.snapshot),
                    metadata={"_chronos_end_txn": txn.numeric_id}
                )
            ...
            self.client.upsert(vectors=[
                Vector(
                    ...
                    metadata={
                        **v.metadata,
                        "_chronos_original_id": v.id,
                        "_chronos_begin_txn": txn.numeric_id,
                        "_chronos_end_txn": INFINITY,
                    }
                )
            ])
    ...
    def query(self, vector: List[float], top_k: int, 
        ...
        for match in results:
            orig_id = match.metadata.get("_chronos_original_id", match.id)
    ...
    def _visibility_filter(self, snapshot: TxnSnapshot) -> dict:
        ...
        return {
            "$and": [
                {"$or": [
                    {"_chronos_begin_txn": {"$lt": xmin}},
                    {"_chronos_begin_txn": {"$in": list(snapshot.rc_txns)}},
                    {"_chronos_begin_txn": {"$eq": x}},
                ]},
                {"_chronos_end_txn": {"$gte": xmin}},
                {"_chronos_end_txn": {"$nin": list(snapshot.rc_txns)}},
                {"_chronos_end_txn": {"$ne": x}},
            ]
        }
```
...
Object stores present a unique challenge: objects are typically immutable blobs without inline metadata filtering on `GET` or `LIST` operations. Chronos handles this with **coordinator-side metadata tracking**.
...
│     ┌─────────────────────────────────────────────────────────┐     │
│     │  For each file in /tmp/chronos/<branch_id>/upper/:          │     │
...
│     ┌─────────────────────────────────────────────────────────┐     │
│     │  $ umount /tmp/chronos/<branch_id>/merged                   │     │
│     │  $ rm -rf /tmp/chronos/<branch_id>/                         │     │
│     └─────────────────────────────────────────────────────────┘     │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
Chronos uses **optimistic concurrency control** — transactions execute without acquiring locks and conflicts are detected at commit time:
...
│  │  upper: /tmp/chronos/txn-abc-123/upper/                     │        │
│  │  merged: /tmp/chronos/txn-abc-123/merged/                   │        │
...
│  │  upper: /tmp/chronos/txn-abc-123:sub-001/upper/             │        │
│  │  merged: /tmp/chronos/txn-abc-123:sub-001/merged/           │        │
...
│  │  upper: /tmp/chronos/txn-abc-123:sub-001:sub-002/upper/     │        │
│  │  merged: /tmp/chronos/txn-abc-123:sub-001:sub-002/merged/   │        │
...
class Subtransaction:
    ...
    async def begin(self, fs_shim: OverlayFSShim) -> None:
        ...
        # Now /tmp/chronos/<branch_id>/merged/ has:
...
### 9.2 Chronos's Context Efficiency Benefits

```
┌─────────────────────────────────────────────────────────────────────┐
│              CONTEXT WINDOW USAGE: BEFORE vs AFTER Chronos              │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  WITHOUT Chronos (Traditional Agent):                                   │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │████████████████████████████████████████████████████████████│    │
...
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
│  WITH Chronos (Transactional Agent):                                    │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │████████████████████████████████████████████████████████████│    │
│  │ Chronos   │        Actual Work         │   Actual Reasoning    │    │
│  │ Calls │          (45%)             │       (50%)           │    │
│  │ (5%)  │                            │                       │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                                                                     │
...
| Metric | Without Chronos | With Chronos | Improvement |
|--------|-------------|----------|-------------|
...
| **State Mirroring** | Shadow state in Chronos-controlled store | Overhead, consistency risk |
...
class MVCCGarbageCollector:
    ...
    def gc_per_shim_type(self) -> dict:
        ...
        """
        return {
            "SQL": "DELETE FROM table WHERE _chronos_end_txn < ? "
                   "OR _chronos_begin_txn IN (?)",
            "Vector": "client.delete(filter={'$or': ["
                      "{'_chronos_end_txn': {'$lt': watermark}}, "
                      "{'_chronos_begin_txn': {'$in': aborted}}]})",
...
| **Trigger-Based** | Install DB triggers to flag non-Chronos modifications | Invasive, performance impact |
| **Periodic Reconciliation** | Compare expected state vs. actual state periodically | Detects but doesn't prevent |
| **Access Control** | Restrict direct DB access, all queries through Chronos | Operationally challenging |
