# Janus Academic Prototype Implementation Plan

## Overview

This document outlines a pragmatic implementation plan for building a **Transactional Agent Runtime (Janus)** prototype suitable for academic research and publication. The goal is to demonstrate the key research contributions while maintaining a manageable scope.
...
│       │   └── exceptions.py      # Janus-specific exceptions
│       │
...
│       │   ├── __init__.py
│       │   ├── tools.py           # Janus tools exposed to LLM
...
│   │
│   ├── benchmarks/
│   │   ├── bench_overhead.py      # Measure Janus overhead vs baseline
...
│
└── docs/
    ├── Janus-design.md              # Design document
    ├── Janus-implementation-plan.md # This document
...
@dataclass
class Transaction:
    """A Janus transaction with virtual branching."""
    id: str
    branch_id: BranchId
...
```python
# src/janus/coordinator/coordinator.py
...
from janus.core.types import Transaction, TransactionState, Savepoint, BranchId
from janus.core.interfaces import TMCPShim, Vote, CoordinatorProtocol
from janus.core.exceptions import (
...
class TransactionCoordinator(CoordinatorProtocol):
    """
    Central coordinator for Janus transactions.
...
```python
# src/janus/shims/sqlite_shim.py
...
from janus.core.interfaces import TMCPShim, Vote
from janus.core.types import Transaction, Savepoint, BranchId
...
class SQLiteShim(TMCPShim):
    ...
    def _setup_schema(self):
        ...
        self._conn.executescript("""
            ...
            CREATE INDEX IF NOT EXISTS idx_janus_branch 
            ON tar_data (branch_id, timestamp);
            
            CREATE INDEX IF NOT EXISTS idx_janus_key 
...
```python
# src/janus/shims/filesystem_shim.py
...
from janus.core.interfaces import TMCPShim, Vote
from janus.core.types import Transaction, Savepoint
...
### 5.1 Janus Tools for LLM

```python
# src/janus/agent/tools.py

from typing import Any, Dict, List, Optional
from dataclasses import dataclass

from janus.coordinator.coordinator import TransactionCoordinator
from janus.core.types import Transaction

@dataclass
class JanusToolResult:
    success: bool
    data: Any = None
...
class JanusTools:
    """
    High-level Janus tools exposed to the LLM agent.
    ...
    async def tar_begin(self) -> JanusToolResult:
        """Start a new transaction. Returns transaction ID."""
        try:
            self._current_txn = await self._coordinator.begin_transaction()
            return JanusToolResult(
                success=True,
                data={"txn_id": self._current_txn.id}
            )
        except Exception as e:
            return JanusToolResult(success=False, error=str(e))
    
    async def tar_checkpoint(self, name: str) -> JanusToolResult:
        """Create a named checkpoint you can rollback to."""
        if not self._current_txn:
            return JanusToolResult(success=False, error="No active transaction")
    ...
    async def tar_rollback(self, to_checkpoint: Optional[str] = None) -> JanusToolResult:
        """
        Rollback changes.
        ...
        try:
            ...
            return JanusToolResult(success=True)
        except Exception as e:
            return JanusToolResult(success=False, error=str(e))
    
    async def tar_commit(self) -> JanusToolResult:
    ...
    async def tar_status(self) -> JanusToolResult:
...
class JanusContextOptimizedTools(JanusTools):
    """
    ...
    async def tar_safe_execute(
        ...
        try:
            ...
            return JanusToolResult(success=True, data=result)
...
```python
# src/janus/agent/adapters/langchain.py

from langchain.tools import BaseTool
from langchain.pydantic_v1 import BaseModel, Field
from typing import Optional, Type

from janus.agent.tools import JanusTools

class JanusBeginInput(BaseModel):
    """Input for tar_begin - no parameters needed."""
    pass

class JanusBeginTool(BaseTool):
    name = "tar_begin"
    description = """Start a new transaction. Call this before making any changes.
    Returns a transaction ID."""
    args_schema: Type[BaseModel] = JanusBeginInput
    
    tar_tools: JanusTools = None
...
class JanusCheckpointInput(BaseModel):
    name: str = Field(description="Name for this checkpoint")

class JanusCheckpointTool(BaseTool):
    name = "tar_checkpoint"
    ...
    args_schema: Type[BaseModel] = JanusCheckpointInput
...
class JanusRollbackTool(BaseTool):
    ...
    args_schema: Type[BaseModel] = JanusRollbackInput
...
def create_janus_langchain_tools(tar_tools: JanusTools) -> list:
    """Create LangChain tools from Janus tools."""
    return [
        JanusBeginTool(tar_tools=tar_tools),
        JanusCheckpointTool(tar_tools=tar_tools),
        JanusRollbackTool(tar_tools=tar_tools),
        JanusCommitTool(tar_tools=tar_tools),
    ]
```
...
|----|----------|--------|------------|
| RQ1 | What is the runtime overhead of Janus? | Latency, throughput | exp1_overhead |
| RQ2 | Does Janus improve context efficiency? | Token count, task success | exp2_context |
| RQ3 | Does Janus provide correct isolation? | Anomaly count | exp3_isolation |
| RQ4 | How well does Janus recover from failures? | Recovery success rate | exp4_recovery |
...
Compare Janus-wrapped operations vs direct operations.
"""
...
from janus.coordinator.coordinator import TransactionCoordinator
from janus.shims.sqlite_shim import SQLiteShim
...
async def main():
    ...
    for r in results:
        ...
        print(f"  Janus: {r.tar_latency_ms:.3f} ms")
...
Compare token usage with and without Janus for the same tasks.
"""
...
class ContextMetrics:
    task_name: str
    without_janus_tokens: int
    with_janus_tokens: int
    without_janus_success: bool
    with_janus_success: bool
    
    @property
    def token_reduction_percent(self) -> float:
        return ((self.without_janus_tokens - self.with_janus_tokens) 
                / self.without_janus_tokens * 100)
...
# Compare token counts between Janus and non-Janus versions
```
...
import asyncio
from janus.coordinator.coordinator import TransactionCoordinator
from janus.shims.filesystem_shim import FileSystemShim
...
*Document created for Janus academic prototype*
