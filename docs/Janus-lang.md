# Janus Implementation in LangChain / LangGraph

**Version:** 0.1.0 · February 2026

This document describes the implementation of Transactional Agent Runtime (Janus) 
...
Janus is implemented as two complementary packages:
...
| **langchain-janus** | `langchain/libs/partners/Janus/langchain_janus/` | LangChain tools that expose Janus to LLM agents |

```
┌─────────────────────────────────────────────────────────────────┐
│                    LLM  Agent  (ReAct loop)                     │
│  tools: JanusFileEditor · JanusBash · JanusSQLite · JanusMemory        │
│         JanusVectorStore                                          │
├──────────────────────┬──────────────────────────────────────────┤
│  JanusContext          │  TransactionCoordinator                  │
│  (langchain_janus)     │  (langgraph.transaction)                │
├──────────────────────┴──────────────────────────────────────────┤
│                        ToolShim  Interface                      │
│  ┌────────────┐ ┌────────────┐ ┌────────────┐                  │
...
Janus implements an **Epoxy-inspired MVCC** (Multi-Version Concurrency Control) protocol 
...
### 7.1 JanusContext (Orchestrator)

**Location:** [langchain_janus/context.py](../langchain/libs/partners/Janus/langchain_janus/context.py)

`JanusContext` is the main entry point that orchestrates the transactional lifecycle:
...
ctx.begin()
tools = ctx.get_tools()  # [JanusFileEditor, JanusMemory, JanusBash, JanusSQLite?, JanusVectorStore?]
...
**Location:** [langchain_janus/file_editor.py](../langchain/libs/partners/Janus/langchain_janus/file_editor.py)
...
**Location:** [langchain_janus/bash.py](../langchain/libs/partners/Janus/langchain_janus/bash.py)

Execute shell commands inside the transactional overlay filesystem.

```python
bash = JanusBash(working_dir=overlay_merged_path)
...
**Location:** [langchain_janus/memory.py](../langchain/libs/partners/Janus/langchain_janus/memory.py)
...
**Location:** [langchain_janus/sqlite_tool.py](../langchain/libs/partners/Janus/langchain_janus/sqlite_tool.py)
...
### 7.6 JanusVectorStore

**Location:** [langchain_janus/vectorstore_tool.py](../langchain/libs/partners/Janus/langchain_janus/vectorstore_tool.py)
...
```
Agent                    JanusContext        Coordinator         OverlayFS     SQLite
  |                          |                  |                  |            |
  |-- begin() ------------->|                  |                  |            |
  |                          |-- begin() ----->|                  |            |
  |                          |                  |-- begin() ----->| mount      |
...
1. **Bolt-on, not baked-in.** Janus wraps unmodified backends via shims; 
...
3. **Same tool semantics.** `JanusFileEditor` behaves identically to a plain 
...
| [langchain_janus/context.py](../langchain/libs/partners/Janus/langchain_janus/context.py) | `JanusContext` orchestrator |
| [langchain_janus/file_editor.py](../langchain/libs/partners/Janus/langchain_janus/file_editor.py) | `JanusFileEditor` tool |
| [langchain_janus/bash.py](../langchain/libs/partners/Janus/langchain_janus/bash.py) | `JanusBash` tool |
| [langchain_janus/memory.py](../langchain/libs/partners/Janus/langchain_janus/memory.py) | `JanusMemory` tool |
| [langchain_janus/sqlite_tool.py](../langchain/libs/partners/Janus/langchain_janus/sqlite_tool.py) | `JanusSQLite` tool |
| [langchain_janus/vectorstore_tool.py](../langchain/libs/partners/Janus/langchain_janus/vectorstore_tool.py) | `JanusVectorStore` tool |

---

*Document created for Janus LangChain/LangGraph implementation reference*
