# Transactional Agent Runtime (Janus) — Technical Summary
...
Janus brings database-grade transactional guarantees to LLM-powered agent
...
│  tools: janus_file_editor · janus_bash · janus_sqlite · janus_memory   │
│         janus_vectorstore · janus_txn                               │
├──────────────────────┬──────────────────────────────────────────┤
...
| Component | Role |
|-----------|------|
| **JanusContext** | Orchestrator that mounts overlays, creates tool instances, and wires them to a shared `TransactionCoordinator`. Exposes `begin()`, `commit()`, `abort()`, `savepoint()`, `rollback()`. |
...
Janus uses an **Epoxy-inspired MVCC** scheme combined with a lightweight
...
For truly non-reversible side-effects (payment charges, sent emails), Janus
...
Janus ships as a LangChain partner package (`langchain-janus`) that exposes
six tools to LLM agents:

| Tool | Description |
|------|-------------|
| `janus_file_editor` | View, create, edit, delete files inside the transactional overlay. |
| `janus_bash` | Run shell commands with `cwd` set to the overlay's merged directory. |
| `janus_sqlite` | MVCC key-value / SQL operations on a transactional SQLite store. |
| `janus_memory` | Persistent agent scratchpad (files under `memories/`), committed or rolled back with the transaction. |
| `janus_vectorstore` | Transactional vector storage — add embeddings, similarity search, delete — all branch-isolated. |
| `janus_txn` | Transaction control: `begin`, `commit`, `abort`, `savepoint <name>`, `rollback`, `status`, `changes`. |

`JanusContext` is the entry point that mounts the overlay, creates all tools
bound to a shared coordinator, and exposes convenience functions:

```python
from langchain_janus import JanusContext, JanusTransactionControl
...
txn_ctl = JanusTransactionControl(ctx) # explicit txn control tool
...
2. **Same tool semantics.**  `janus_file_editor` behaves identically to a
