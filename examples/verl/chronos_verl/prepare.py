...
def initialize(url):
    ...
    try:
        ...
        ctx.db.execute("CREATE TABLE tickets (id INTEGER PRIMARY KEY, subject TEXT, priority TEXT)")
        ctx.db.execute("INSERT INTO tickets VALUES (101, 'Payment outage', 'normal')")
        ctx.db.execute("INSERT INTO tickets VALUES (102, 'Documentation typo', 'normal')")
        ctx.db.commit()
...
def main():
    ...
    if args.prompts:
        ...
        pd.DataFrame([{
            ...
            "reward_model": {"style": "rule", "ground_truth": "ticket_101_high_only"},
