...
from chronos_code.agent.agent_loop import AgentLoop
from chronos_code.agent.external_workers import ExternalWorkerAdapter
from chronos_code.agent.dag import (
...
from chronos_code.agent.sub_agents import (
    SubAgentConfig,
    filter_tools,
...
from chronos_code.chronos_integration.aggregator import Aggregator
...
from chronos_code.chronos_integration.sub_agent_txn import SubAgentTxn, SubAgentResult
from chronos_code.tools.registry import build_tool_schemas
...
class Orchestrator:
    ...
    In Phase 2, this runs without Chronos transaction integration — just
    ...
    def __init__(
        ...
        self._embedded_mcp_server: EmbeddedChronosMcpServer | None = None
        if (
            self._external_worker_adapter.runtime in ("codex", "claude")
            and bool(config.external_worker_use_embedded_mcp)
        ):
            self._embedded_mcp_server = EmbeddedChronosMcpServer(
    ...
    def _resolve_txn_working_dir(
        ...
        if session is not None:
            chronos_ctx = getattr(session, "chronos_context", None)
            workdir = getattr(chronos_ctx, "working_dir", None)
            if workdir is not None:
                return Path(workdir)
        if txn is not None:
            chronos_ctx = getattr(txn, "_chronos_context", None)
            workdir = getattr(chronos_ctx, "working_dir", None)
            if workdir is not None:
                return Path(workdir)
    ...
    async def _run_external_worker(
        ...
        *,
        ...
        if self._embedded_mcp_server is not None:
            chronos_ctx = (
                getattr(session, "chronos_context", None)
                if session is not None
                else getattr(txn, "_chronos_context", None)
            )
            if chronos_ctx is None:
                raise RuntimeError(
                    "Embedded MCP mode requires an active ChronosContext from orchestrator."
                )
            self._embedded_mcp_server.ensure_started()
            self._embedded_mcp_server.register_session(session_id, chronos_ctx)
    ...
    def _create_parallel_worker_session(
        ...
        """Create an isolated Chronos session for one parallel worker attempt."""
        try:
            from langchain_chronos import ChronosContext
        except Exception:
            self._emit_event(
                "parallel_session_fallback",
                "Parallel worker session fallback to shared context (ChronosContext unavailable).",
                task=self._clip(task.description, max_chars=100),
            )
            return base_session

        try:
            parallel_dir = self.working_dir / ".chronos-code" / "parallel"
            ...
            worker_session = SessionManager(
                ...
                chronos_context=worker_ctx,
                db_path=":memory:",
            )
            worker_session.start_session()
            self._emit_event(
                "parallel_session_created",
                "Created isolated worker Chronos session.",
                task=self._clip(task.description, max_chars=100),
                attempt=attempt,
            )
            ...
            self._emit_event(
                "parallel_session_fallback",
                "Failed to create isolated worker Chronos session; using shared session.",
                task=self._clip(task.description, max_chars=100),
                attempt=attempt,
                error=self._clip(str(e), max_chars=180),
            )
    ...
    def _get_parallel_tools_for_session(
        ...
        """Build per-worker tools bound to the worker Chronos context."""
        if self._parallel_tools_for_context is not None:
            try:
                return self._parallel_tools_for_context(session.chronos_context)
            except Exception as e:
                self._emit_event(
        ...
        # Shared fallback (may serialize behavior if using shared ChronosContext).
