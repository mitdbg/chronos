...
Manages begin/commit/abort of Chronos transactions, session metadata,
and the bridge between the agent layer and the Chronos shim infrastructure.

Every begin/commit/abort/savepoint call delegates to ChronosContext and
...
class TxnContext:
    ...
    Delegates lifecycle operations to ChronosContext, including savepoints
    for subtransactions.
    """
    ...
    def begin_subtxn(self, name: str = "") -> "TxnContext":
        ...
        When a ChronosContext is present, creates a real OverlayFS savepoint
        so the sub-agent's writes are isolated. Tools are automatically
        switched to the child overlay by ChronosContext.savepoint().
        """
        self._child_seq += 1
        child_id = f"{self.txn_id}_{self._child_seq}"

        child_handle = None
        if self._chronos_context and self._chronos_context.is_active:
            # Creates child overlay + switches all tools to child workdir
            self._chronos_context.savepoint(name or child_id)
            # The handle for the child is tracked inside ChronosContext
            child_handle = self._chronos_context._child_txn

        child = TxnContext(
            txn_id=child_id,
            txn_handle=child_handle,
            chronos_context=self._chronos_context,  # share the same ChronosContext
        )
    ...
    def commit(self) -> None:
        ...
        if self._chronos_context:
            if self._is_subtxn:
                # Child may already be closed externally; treat as finalized.
                if self._chronos_context._child_txn is None:
                    logger.warning(
                ...
                self._chronos_context._commit_active_savepoint()
            else:
                # Top-level txn may already be committed/aborted externally.
                if not self._chronos_context.is_active:
                    logger.warning(
    ...
    def abort(self) -> None:
        ...
        if self._chronos_context:
            if self._is_subtxn:
                ...
                if self._chronos_context._child_txn is not None:
                    self._chronos_context.rollback()
            else:
                # Top-level abort: discard all overlay changes
                if self._chronos_context.is_active:
                    self._chronos_context.abort()
...
class SessionManager:
    ...
    Args:
        ...
        chronos_context: ChronosContext instance used for all transactions.
    ...
    def __init__(
        ...
        if chronos_context is None:
            raise ValueError("SessionManager requires an active ChronosContext.")
        self.project_path = project_path
        self.config = config
        self.chronos_context = chronos_context
        self.session_id: str = ""
        self.turn_number: int = 0
        self._current_txn: TxnContext | None = None

        # Message persistence always participates in Chronos's 2PC via SQLiteShim.
        shim = getattr(chronos_context, "_sqlite_shim", None)
        if shim is None:
            raise ValueError(
                "SessionManager requires ChronosContext with SQLiteShim enabled."
            )
    ...
    def start_session(self) -> str:
        ...
        In Chronos mode, the session metadata row is written in a brief
        ...
        bootstrap_handle = self.chronos_context.begin()
    ...
    def resume_session(self, session_id: str) -> list[Message]:
        ...
        In Chronos mode, opens a read-only transaction to query committed
    ...
    def _load_messages_for_session(self, session_id: str) -> list[Message]:
        """Load session messages, opening a short read txn in Chronos mode if needed."""
        if self.message_store._txn is None:
            ...
            try:
                ...
                if self.chronos_context.is_active:
    ...
    def begin_txn(self) -> TxnContext:
        ...
        If a ChronosContext was provided, calls chronos_context.begin() which
        ...
        txn_handle = self.chronos_context.begin()
        ...
        txn = TxnContext(
            ...
            chronos_context=self.chronos_context,
        )
    ...
    def abort_txn(self, txn: TxnContext | None = None) -> None:
        ...
        In Chronos mode the MVCC shim eagerly undoes the message rows written
    ...
    def begin_parallel_txns(self, n: int) -> list[TxnContext]:
        ...
        Note: for true concurrent overlays, use per-task ChronosContext
