...
class MessageStore:
    ...
    the Chronos SQLiteShim and require a current transaction handle.

    Args:
        db_path: Retained for compatibility with call sites; unused.
        shim: ``SQLiteShim`` from the active ``ChronosContext``.
    ...
    def __init__(
        ...
        self._shim = shim
        self._txn = txn
        # Per-process sequence counters for synthetic PKs (Chronos mode).
        self._msg_seq = 0
        self._tc_seq = 0
