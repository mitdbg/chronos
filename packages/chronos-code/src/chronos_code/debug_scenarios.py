...
These scenarios exercise Chronos transaction semantics without relying on
...
from chronos_code.agent.dag import normalize_planner_dag
from chronos_code.chronos_integration.session_manager import SessionManager


class DebugScenarioRunner:
    """Run deterministic Chronos scenarios for CLI verification."""
    ...
    def _subagent_speculative(self) -> str:
        ...
        try:
            ...
            (self.session.chronos_context.working_dir / "debug_calc.py").write_text(wrong_fix)
            attempt_a.abort()

            parent_after_abort = (
                self.session.chronos_context.working_dir / "debug_calc.py"
            ).read_text()
            rollback_ok = parent_after_abort == buggy

            attempt_b = txn.begin_subtxn("attempt_b_correct_fix")
            (self.session.chronos_context.working_dir / "debug_calc.py").write_text(correct_fix)
            attempt_b.commit()
    ...
    def _parallel_snapshot(self) -> str:
        """Show snapshot isolation across concurrent Chronos transactions."""
        ...
        chronos_dir = self.working_dir / ".chronos-code"
        chronos_dir.mkdir(exist_ok=True)
        db_path = str(chronos_dir / "debug_snapshot.db")
    ...
    def _parallel_db_conflict_retry(self) -> str:
        ...
        from chronos_core.transaction.shim_sqlite import SQLiteShim, WriteConflictError

        chronos_dir = self.working_dir / ".chronos-code"
        chronos_dir.mkdir(exist_ok=True)
        db_path = str(chronos_dir / "debug_parallel_conflict.db")
    ...
    def _nested_dag_wave(self) -> str:
        ...
        def _run_node(node_id: str) -> str:
            db_path = self.working_dir / ".chronos-code" / f"debug_node_{node_id}.db"
            ctx = ChronosContext(
                str(self.working_dir),
                db_path=str(db_path),
                enable_sqlite=True,
