"""Bootstrap helpers for configuring Codex/Claude to use Chronos MCP."""
...
def bootstrap_project(
    *,
    ...
    mcp_command: str = "chronos-code-mcp",
    ...
    bootstrap_root = resolved_project_dir / ".chronos-code" / "bootstrap"
    if runtime in ("codex", "both"):
        codex_dir = bootstrap_root / "codex"
        result.written_files.append(
            _write_text(
                codex_dir / "AGENTS.chronos.md",
                build_agents_instruction(server_name=server_name),
            )
        )
        result.written_files.append(
            _write_text(
                codex_dir / "skills" / "chronos-transaction-protocol" / "SKILL.md",
        ...
        result.written_files.append(
            _write_text(
                ...
                build_codex_skill_install_script(
                    project_dir=resolved_project_dir,
                    skill_rel_path=Path(".chronos-code/bootstrap/codex/skills/chronos-transaction-protocol"),
                ),
                executable=True,
            )
        )
...
def build_agents_instruction(*, server_name: str) -> str:
    return f"""# Chronos MCP Workflow (Codex)

Use MCP server `{server_name}` for filesystem/database operations.

1. Start each task with `chronos_txn` action `begin` and set a stable `session_id`.
2. Use the same `session_id` for all `chronos_file_editor`, `chronos_bash`, and `chronos_sqlite` calls.
3. Before finalizing, call `chronos_txn` action `changes` and `status`.
4. If checks fail, call `chronos_txn` action `abort`; otherwise call `commit`.
5. Never commit when tests or validation are failing.

If the client namespaces MCP tools, the tool names may appear as:
`mcp__{server_name}__chronos_txn`, `mcp__{server_name}__chronos_file_editor`, etc.
"""


def build_claude_instruction(*, server_name: str) -> str:
    return f"""# Chronos MCP Workflow (Claude)

Use MCP server `{server_name}` for transactional file/database actions.

1. Begin with `chronos_txn` action `begin` using a consistent `session_id`.
2. Keep all tool calls for the task on that same `session_id`.
3. Use `chronos_txn` action `changes` before finalizing.
...
def build_skill_md(*, server_name: str) -> str:
    return f"""---
name: chronos-transaction-protocol
description: Use Chronos MCP tools safely with explicit transaction boundaries.
---

# Chronos Transaction Protocol

When MCP server `{server_name}` is available:

- Always call `chronos_txn` with `action="begin"` first.
- Reuse one `session_id` across all Chronos calls in a task.
- Use `chronos_file_editor`/`chronos_bash`/`chronos_sqlite` for changes and checks.
- Call `chronos_txn` with `action="changes"` before finalizing.
- If anything is wrong, call `chronos_txn` with `action="abort"`.
- Call `chronos_txn` with `action="commit"` only after validations pass.
"""
...
SKILL_SRC="{skill_src}"
SKILL_DST="${{CODEX_HOME:-$HOME/.codex}}/skills/chronos-transaction-protocol"
...
# Registers project-scoped Chronos MCP server in Claude Code.
