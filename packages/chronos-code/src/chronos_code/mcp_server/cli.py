"""CLI entrypoint for running Chronos as an MCP server."""
...
from chronos_code.mcp_server.server import ChronosMcpServeConfig, run_mcp_server
...
@click.option(
    ...
    "-p",
    default=".",
    help="Project directory for Chronos OverlayFS base (default: current directory).",
)
@click.option(
    "--db-path",
    default=None,
    help="SQLite database path for Chronos transactional state (default: .chronos-code/mcp.sqlite).",
)
@click.option(
    "--transport",
...
@click.option(
    "--disable-sqlite",
    is_flag=True,
    help="Disable chronos_sqlite tool exposure.",
)
@click.option(
    "--enforced-session-id",
    default=None,
    help=(
        "If set, this server only accepts this Chronos session_id (and maps default "
...
def main(
    ...
    """Run Chronos as an MCP server for Codex/Claude-compatible clients."""
