"""Embedded Chronos MCP server bound to orchestrator-managed transactions."""
...
class EmbeddedChronosMcpServer:
    """Runs Chronos MCP server in-process and routes calls to registered contexts.
    ...
    ChronosContext instances that are already owned by Chronos-code orchestrator.
    """
    ...
    def register_session(self, session_id: str, chronos_context: Any) -> None:
        with self._lock:
            self._contexts[session_id] = chronos_context
            ...
            def chronos_txn(
                ...
                def _execute() -> str:
                    ...
                    if action in {"commit", "abort", "savepoint", "rollback"}:
                        ...
                        return (
                            f"chronos_txn action '{action}{suffix}' acknowledged as no-op in embedded mode. "
                            "Transaction boundaries are controlled by Chronos-code orchestrator."
                        )
                    return (
    ...
    def _get_context(self, session_id: str) -> Any:
        ...
        if ctx is None:
            raise RuntimeError(
                f"Unknown Chronos MCP session '{sid}'. "
    ...
    def _clip_json(payload: dict[str, Any], max_chars: int = 180) -> str:
        ...
        return EmbeddedChronosMcpServer._clip(rendered, max_chars=max_chars)
