"""Runtime bridge between MCP tool calls and Chronos transactional tools."""
...
class ChronosMcpRuntimeConfig:
    """Configuration for MCP-backed Chronos runtime sessions."""
...
class ChronosMcpRuntime:
    """Manages per-session Chronos contexts for MCP tools.

    A "session" here is a logical agent identity provided by MCP tool
    arguments. Each session gets its own ChronosContext instance and transaction
    lifecycle.
    """

    def __init__(
        self,
        config: ChronosMcpRuntimeConfig,
        *,
        context_factory: ContextFactory | None = None,
    ) -> None:
        self._project_dir = Path(config.project_dir).resolve()
        self._state_dir = self._project_dir / ".chronos-code"
    ...
    def chronos_txn(
        ...
        *,
        ...
        """Control Chronos transaction lifecycle for one logical session."""
        try:
            ...
            if action == "begin":
                ...
                return (
                    ...
                    "Use chronos_txn(action='commit') to apply or chronos_txn(action='abort') to discard."
                )
    ...
    def chronos_file_editor(
        ...
        *,
        ...
        """Execute a Chronos file editor command against the session overlay."""
        try:
            sid = self._effective_session_id(session_id)
    ...
    def chronos_sqlite(
        ...
        *,
        ...
        """Execute Chronos SQLite command within the active transaction."""
        try:
            sid = self._effective_session_id(session_id)
            ctx = self._active_context(sid)
            sqlite_tool = getattr(ctx, "sqlite", None)
            if sqlite_tool is None:
                return "Error: chronos_sqlite is disabled for this runtime."
    ...
    def _ensure_active(ctx: Any, session_id: str) -> None:
        ...
        raise RuntimeError(
            ...
            "Call chronos_txn(action='begin') first."
        )
    ...
    def _default_context_factory(base_path: Path, **kwargs: Any) -> Any:
        ...
        return ChronosContext(base_path, **kwargs)
