...
class OverlayFSShim(ToolShim):
    ...
    def __init__(
        ...
        # /tmp/chronos_overlayfs which may be root-owned from older runs.
        if chronos_root is not None:
            self._chronos_root = Path(chronos_root)
        elif os.environ.get("CHRONOS_ROOT"):
            self._chronos_root = Path(os.environ["CHRONOS_ROOT"])
        else:
            uid = os.getuid()
            self._chronos_root = Path(f"/tmp/chronos_overlayfs_u{uid}")
        self._chronos_root.mkdir(mode=0o700, parents=True, exist_ok=True)
        # Backward-compatible attribute used by existing test fixtures.
        self.CHRONOS_ROOT = self._chronos_root
    ...
    def begin(self, txn: TransactionHandle) -> None:
        ...
        branch_root = self._chronos_root / bid
    ...
    def begin_child(
        ...
        branch_root = self._chronos_root / child_bid
        upper = branch_root / "upper"
        work = branch_root / "work"
        merged = branch_root / "merged"
        savepoints_dir = branch_root / "savepoints"
