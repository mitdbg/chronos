"""Postgres MVCC shim for transactional virtual branching.

Implements the Epoxy-style MVCC protocol on top of Postgres:
...
class PostgresShim(ToolShim):
    """MVCC-based Postgres shim for transactional virtual branching.
    ...
    Example::
        ...
        shim = PostgresShim("postgresql://localhost/postgres")
        shim.register_table("users", ["id TEXT", "name TEXT", "email TEXT"])
    ...
    @classmethod
    def _column_name_from_def(cls, column_def: str) -> str:
        parts = column_def.strip().split(maxsplit=1)
        if not parts:
            raise ValueError("Empty column definition")
        name = parts[0]
        # Keep the input format simple and unquoted. This mirrors the SQLite
        # shim API but quotes identifiers safely when generating PostgreSQL SQL.
        cls._quote_ident(name)
        return name

    @classmethod
    def _column_def_sql(cls, column_def: str) -> str:
        parts = column_def.strip().split(maxsplit=1)
        if not parts:
            raise ValueError("Empty column definition")
        name = parts[0]
        rest = parts[1] if len(parts) > 1 else "TEXT"
        return f"{cls._quote_ident(name)} {rest}"

    @staticmethod
    def _visibility_sql(snapshot: TxnSnapshot) -> tuple[str, list[int]]:
        clause, params = snapshot.visibility_sql()
    ...
    def _column_sql(self, meta: _TableMeta, column_name: str) -> str:
        if column_name not in meta.columns and column_name not in {
            "_begin_txn",
            "_end_txn",
            "_row_version",
            "ctid",
            ...
            raise ValueError(
                f"Unknown column {column_name!r} for table {meta.name!r}"
            )
        return self._quote_ident(column_name)
    ...
    def register_table(
        ...
        col_names = [self._column_name_from_def(c) for c in columns]

        pk = pk_column or col_names[0]
        self._quote_ident(pk)
        meta = _TableMeta(
            name=table_name,
            columns=col_names,
            column_defs=columns,
        ...
        all_cols = [self._column_def_sql(c) for c in columns] + [
        ...
        index_name = self._quote_ident(
            f"idx_{table_name.replace('.', '_')}_mvcc"
        )
        with self._lock:
            self.conn.execute(
                f"CREATE TABLE IF NOT EXISTS {table_sql} ({col_sql})"
            )
            # Index on MVCC columns for predicate push-down
            self.conn.execute(
                f"CREATE INDEX IF NOT EXISTS {index_name} "
                f"ON {table_sql}(_begin_txn, _end_txn)"
            )
            self.conn.commit()
    ...
    def seed_data(
        ...
        col_str = ", ".join(self._quote_ident(c) for c in cols)
        table_sql = self._quote_ident(table_name)
        with self._lock:
            for row in rows:
                ...
                self.conn.execute(
                    f"INSERT INTO {table_sql} ({col_str}) VALUES ({placeholders})",
                    vals,
                )
            self.conn.commit()
    ...
    def prepare(self, txn: TransactionHandle) -> Vote:
        ...
        with self._lock:
            for table_name, pk_val in ws:
                ...
                pk_sql = self._column_sql(meta, meta.pk)
                cur = self.conn.execute(
                    f"SELECT _begin_txn, _end_txn FROM {table_sql} "
                    f"WHERE {pk_sql} = %s",
                    [pk_val],
                )
    ...
    def abort(self, txn: TransactionHandle) -> None:
        ...
        with self._lock:
            for table_name, pk_val in ws:
                ...
                self.conn.execute(
                    f"UPDATE {table_sql} SET _end_txn = 0 "
                    f"WHERE _end_txn = %s AND {pk_sql} = %s",
                    [txn.numeric_id, pk_val],
                )
                # Delete records this txn created
                self.conn.execute(
                    f"DELETE FROM {table_sql} "
                    f"WHERE _begin_txn = %s AND {pk_sql} = %s",
                    [txn.numeric_id, pk_val],
                )
            self.conn.commit()
    ...
    def put(
        ...
        with self._lock:
            ...
            if txn.snapshot:
                vis_clause, vis_params = self._visibility_sql(txn.snapshot)
                cur = self.conn.execute(
                    f"SELECT ctid AS _chronos_ctid, _begin_txn, _end_txn FROM {table_sql} "
                    f"WHERE {pk_sql} = %s AND {vis_clause} "
                ...
                if existing:
                    ...
                    self.conn.execute(
                        f"UPDATE {table_sql} SET _end_txn = %s WHERE ctid = %s",
                        [write_txn_id, existing["_chronos_ctid"]],
                    )
    ...
    def increment(
        ...
        *,
        ...
        with self._lock:
            cur = self.conn.execute(
                f"SELECT ctid AS _chronos_ctid, * FROM {table_sql} "
                f"WHERE {pk_sql} = %s AND _end_txn = 0 "
    ...
    def query(
        ...
        table_sql = self._table_sql(table)
        pk_sql = self._column_sql(meta, meta.pk)
        select_cols = ", ".join(self._column_sql(meta, c) for c in meta.columns)
        ...
        if filters:
            for col, val in filters.items():
                where_parts.append(f"{self._column_sql(meta, col)} = %s")
                params.append(val)

        where_sql = " AND ".join(where_parts)

        inner_sql = (
            f"SELECT DISTINCT ON ({pk_sql}) * FROM {table_sql} "
            f"WHERE {where_sql} "
            f"ORDER BY {pk_sql}, _begin_txn DESC"
        )
        sql = f"SELECT {select_cols} FROM ({inner_sql}) AS _chronos_visible"
        if order_by:
            sql += f" ORDER BY {self._column_sql(meta, order_by)}"
        ...
        with self._lock:
            ...
            rows = cur.fetchall()
        return [{col: row[col] for col in meta.columns} for row in rows]
    ...
    def delete(
        ...
        with self._lock:
            cur = self.conn.execute(
                f"SELECT ctid AS _chronos_ctid, _end_txn FROM {table_sql} "
                f"WHERE {pk_sql} = %s AND {vis_clause} "
                f"ORDER BY _begin_txn DESC LIMIT 1",
                [pk_val] + vis_params,
            )
            ...
            self.conn.execute(
                f"UPDATE {table_sql} SET _end_txn = %s WHERE ctid = %s",
                [txn.numeric_id, existing["_chronos_ctid"]],
            )
            self.conn.commit()
    ...
    def gc(self, committed_below: int) -> int:
        ...
        with self._lock:
            for table_name in self._tables:
                ...
                cur = self.conn.execute(
                    f"DELETE FROM {table_sql} "
                    f"WHERE _end_txn != 0 AND _end_txn < %s",
                    [committed_below],
                )
                total += cur.rowcount
            self.conn.commit()
