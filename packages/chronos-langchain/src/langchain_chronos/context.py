"""Chronos Context — transactional wrapper for tool calls.

Provides ``ChronosContext``, a context manager that orchestrates the full
transactional lifecycle around Chronos tools:
...
As a context manager::
    ...
    with ChronosContext("/path/to/project") as ctx:
        tools = ctx.get_tools()
        # ... agent uses tools ...
...
from langchain_chronos.vectorstore_tool import ChronosVectorStore
...
class ChronosContext:
    ...
    def __init__(
        ...
        self._file_editor: ChronosFileEditor | None = None
        self._memory: ChronosMemory | None = None
        self._bash: ChronosBash | None = None
        self._sqlite: ChronosSQLite | None = None
        self._vectorstore: ChronosVectorStore | None = None
    ...
    @property
    def file_editor(self) -> ChronosFileEditor:
    ...
    @property
    def vectorstore(self) -> ChronosVectorStore:
    ...
    def begin(self) -> TransactionHandle:
        ...
        self._file_editor = ChronosFileEditor(working_dir=workdir)
        self._memory = ChronosMemory(working_dir=workdir)
        self._bash = ChronosBash(working_dir=workdir)
        if self._sqlite_shim is not None:
            self._sqlite = ChronosSQLite(shim=self._sqlite_shim, txn=self._txn)
        if self._vec_shim is not None:
            self._vectorstore = ChronosVectorStore(shim=self._vec_shim, txn=self._txn)
        self._tools_created = True

        logger.info("Chronos transaction begun (txn=%s, workdir=%s)", self._txn.id, workdir)
        return self._txn
    ...
    def commit(self) -> None:
        ...
        logger.info("Chronos transaction committed (txn=%s)", self._txn.id)
        self._cleanup_tools()
    ...
    def abort(self) -> None:
        ...
        logger.info("Chronos transaction aborted (txn=%s)", self._txn.id)
        self._cleanup_tools()
    ...
    def savepoint(self, name: str) -> None:
        ...
        logger.info(
            "Chronos savepoint '%s' created (child=%s)", name, self._child_txn.id
        )
    ...
    def rollback(self, name: str | None = None) -> None:
        ...
        self._coordinator.abort_child(self._child_txn.id)
        logger.info(
            "Chronos savepoint rolled back (child=%s, name=%s)",
            self._child_txn.id,
            name,
        )
        self._child_txn = None
    ...
    def get_tools(self) -> list[BaseTool]:
        """Return all Chronos tools bound to the current transaction.

        Returns:
            List of tools: [ChronosFileEditor, ChronosMemory, ChronosBash] plus
            ChronosSQLite and ChronosVectorStore if enabled.
    ...
    def __enter__(self) -> ChronosContext:
        self.begin()
        return self
    ...
    def _commit_active_savepoint(self) -> None:
        ...
        if self._child_txn is not None and self._child_txn.is_active:
            ...
            self._child_txn = None
            self._restore_tools_to_parent()
            logger.info("Chronos active savepoint committed into parent txn")
...
# ── Chronos Transaction Control Tool ─────────────────────────────────────


class ChronosTransactionControlInput(BaseModel):
...
class ChronosTransactionControl(BaseTool):
    """Tool that exposes Chronos transaction control to the model.
    ...
    Example::
        ...
        ctx = ChronosContext("/path/to/project")
        txn_ctl = ChronosTransactionControl(chronos_context=ctx)
    ...
    args_schema: type[BaseModel] = ChronosTransactionControlInput

    chronos_context: ChronosContext
    """The ChronosContext to control."""
    ...
    def _run(
        ...
        try:
            if action == "begin":
                txn = self.chronos_context.begin()
                return (
                ...
                self.chronos_context.savepoint(name)
                return (
                ...
                self.chronos_context.rollback(name)
                return (
    ...
    def _get_status(self) -> str:
        ...
        if not self.chronos_context.is_active:
            return "No active transaction."

        txn = self.chronos_context.txn
        assert txn is not None
        workdir = self.chronos_context.working_dir
        changes = self.chronos_context.get_changes()
...
def create_chronos_tools(
    base_path: str | Path,
    *,
    weak_snapshot: bool = False,
) -> tuple[ChronosContext, list[BaseTool]]:
    """Convenience: create a ChronosContext and return all tools including txn control.
    ...
    Note:
        ...
        or let the model use the ``chronos_txn`` tool with action='begin'.
    """
    ctx = ChronosContext(base_path, weak_snapshot=weak_snapshot)
    txn_ctl = ChronosTransactionControl(chronos_context=ctx)
...
def create_chronos_tools_with_active_txn(
    base_path: str | Path,
    *,
    weak_snapshot: bool = False,
) -> tuple[ChronosContext, list[BaseTool]]:
    """Create a ChronosContext, begin a transaction, and return all tools.
