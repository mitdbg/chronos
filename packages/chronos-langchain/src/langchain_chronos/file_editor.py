"""Chronos File Editor tool — transactional file operations on OverlayFS.
...
class ChronosFileEditorInput(BaseModel):
    """Input schema for the Chronos file editor tool."""
...
class ChronosFileEditor(BaseTool):
    ...
    or ``ChronosContext.working_dir``).

    Example::

        editor = ChronosFileEditor(working_dir=Path("/tmp/tar_overlayfs/.../merged"))
    ...
    args_schema: type[BaseModel] = ChronosFileEditorInput
