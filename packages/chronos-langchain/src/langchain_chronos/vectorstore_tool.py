"""Chronos VectorStore tool — transactional MVCC vector storage for RAG.

Wraps ``SqliteVecShim`` from Chronos's transaction layer to provide
...
Example::
    ...
    vec = ChronosVectorStore(shim=vec_shim, txn=txn)
    vec.invoke({
...
class ChronosVectorStoreInput(BaseModel):
    """Input schema for the Chronos VectorStore tool."""
...
class ChronosVectorStore(BaseTool):
    ...
    args_schema: type[BaseModel] = ChronosVectorStoreInput
...
class ChronosTransactionalVectorStore:
    """LangChain VectorStore-compatible interface wrapping ChronosVectorStore.
    ...
    Example::
        ...
        from langchain_chronos.vectorstore_tool import ChronosTransactionalVectorStore
        
        # Create with explicit shim+txn (from ChronosContext)
        vs = ChronosTransactionalVectorStore(shim=ctx.vec_shim, txn=ctx.txn, collection="docs")
