...
to Janus MCP server with an enforced session id.
"""
...
class ExternalWorkerAdapter:
    ...
    _DEFAULT_MCP_TOOLS = ["janus_txn", "janus_file_editor", "janus_bash", "janus_sqlite"]
    ...
    async def run_worker(
        ...
        *,
        ...
        run_dir = self._root_dir / ".janus-code" / "external-workers" / run_id
    ...
    def _build_prompt(
        ...
        *,
        ...
        txn_rules = (
            ...
            "- Do not call janus_txn commit/abort/savepoint/rollback.\n"
            "- You may call janus_txn status/changes for inspection.\n"
            if mcp_url
            else
            "- Begin with janus_txn action=begin.\n"
            "- Execute the contract, verify results, then janus_txn changes/status.\n"
            "- If checks fail, janus_txn abort. If checks pass, janus_txn commit.\n"
        )
        return (
            f"You are a {runtime} worker ({agent_type}) launched by Janus-code.\n\n"
            "Rules:\n"
            f"- Use MCP server '{server_name}' only for Janus transactional work.\n"
            f"- The required Janus session_id is '{branch_session_id}'.\n"
            "- This session_id is enforced by the Janus MCP server.\n"
            f"{txn_rules}"
