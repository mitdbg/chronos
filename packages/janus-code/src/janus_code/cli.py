...
def _prepare_janus_state_dir(working_dir: Path, fresh: bool) -> Path:
    """Ensure project-local Janus state directory exists.

    When ``fresh`` is True, remove any existing state first.
    """
    janus_state_dir = working_dir / ".janus-code"
    if fresh and janus_state_dir.exists():
        shutil.rmtree(janus_state_dir)
    janus_state_dir.mkdir(parents=True, exist_ok=True)
    return janus_state_dir
...
def _create_janus_context(working_dir: Path, *, weak_snapshot: bool = False) -> Any:
    """Create a JanusContext for the session.

    Janus-Code is Janus-only and requires a working JanusContext.
    """
    try:
        from langchain_janus import JanusContext
        janus_dir = working_dir / ".janus-code"
        janus_dir.mkdir(exist_ok=True)
        db_path = str(janus_dir / "session.db")
        ctx = JanusContext(
            str(working_dir),
            db_path=db_path,
            enable_sqlite=True,
        ...
        raise click.ClickException(
            "Failed to initialize Janus context. Janus-Code requires Janus-enabled "
            f"execution.\nUnderlying error: {e}\n"
...
async def _repl_loop(
    ...
    from janus_code.display.terminal_ui import TerminalUI
    from janus_code.display.todo_display import TodoDisplay
    from janus_code.debug_scenarios import DebugScenarioRunner
    from janus_code.janus_integration.session_manager import SessionManager
    from janus_code.tools.memory_tool import MemoryTool
    from janus_code.tools.registry import build_tools, build_tool_schemas
    ...
    janus_state_dir = _prepare_janus_state_dir(working_dir, fresh=fresh)
    if fresh:
        ui.print_info(f"Fresh mode: reset state at {janus_state_dir}")

    # Janus-only execution
    janus_ctx = _create_janus_context(
        working_dir,
    ...
    # Initialize session
    session_db_path = str(janus_state_dir / "messages.db")

    session = SessionManager(
        project_path=working_dir,
        config=config,
        janus_context=janus_ctx,
    ...
    try:
        from janus_code.agent.llm import build_llm_fn
    ...
    while True:
        ...
        try:
            ...
            # Safety: ensure JanusContext is clean even if abort_txn missed it
            if janus_ctx and janus_ctx.is_active:
                try:
                    janus_ctx.abort()
                except Exception:
                    pass
...
@click.option(
    "--fresh",
    is_flag=True,
    help="Reset project-local .janus-code state before starting.",
)
@click.option(
    "--weak-snapshot",
    is_flag=True,
...
@click.option(
    ...
    default=True,
    show_default=True,
    help="Use in-process embedded Janus MCP server for external workers.",
)
@click.option(
    "--embedded-mcp-host",
    default="127.0.0.1",
    show_default=True,
    help="Embedded Janus MCP host.",
)
@click.option(
    "--embedded-mcp-port",
    type=int,
    default=8765,
    show_default=True,
    help="Embedded Janus MCP port.",
)
@click.option(
    "--embedded-mcp-path",
    default="/mcp",
    show_default=True,
    help="Embedded Janus MCP HTTP path.",
)
@click.option(
...
def main(
    ...
    """Janus-Code: A transactional CLI coding agent."""
