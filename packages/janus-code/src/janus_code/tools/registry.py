...
The runtime always uses the standard Janus-Code tool names
(`read_file`, `write_file`, `edit_file`, `bash`, `memory`, etc.).
When a JanusContext transaction is active, these tools are pointed at
``janus_context.working_dir`` so they operate inside Janus's OverlayFS
...
def build_tools(
    ...
    """Create all tool instances for Janus-Code.

    Args:
        working_dir: The project root directory.
        config: Runtime configuration.
        janus_context: Optional JanusContext. When provided and active, all
        ...
        expose_txn_control: When True, expose ``janus_txn`` tool for manual
    ...
    """
    from janus_code.tools.read_file import ReadFileTool
    ...
    from janus_code.tools.todo_write import TodoWriteTool
    from janus_code.tools.ask_user import AskUserTool
    from janus_code.tools.memory_tool import MemoryTool
    from janus_code.tools.sqlite_tool import SQLiteToolAdapter
    ...
    workdir = working_dir
    if janus_context is not None and janus_context.is_active:
        workdir = janus_context.working_dir

    # Core filesystem/shell tools always use the standard Janus-Code names.
    ...
    if janus_context is not None and getattr(janus_context, "_sqlite_shim", None) is not None:
        sqlite_tool = SQLiteToolAdapter(janus_context=janus_context)
        _register_tool("sqlite", sqlite_tool, "janus_sqlite")

    # Optional transaction control tool for debug/testing only.
    if janus_context is not None and expose_txn_control:
        from langchain_janus.context import JanusTransactionControl

        txn_ctl = JanusTransactionControl(janus_context=janus_context)
        _register_tool("janus_txn", JanusToolAdapter(txn_ctl))
