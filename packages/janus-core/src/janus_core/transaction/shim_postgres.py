...
class PostgresShim(ToolShim):
    ...
    def put(
        ...
        with self._lock:
            ...
            if txn.snapshot:
                ...
                cur = self.conn.execute(
                    f"SELECT ctid AS _janus_ctid, _begin_txn, _end_txn FROM {table_sql} "
                ...
                if existing:
                    ...
                    self.conn.execute(
                        ...
                        [write_txn_id, existing["_janus_ctid"]],
                    )
    ...
    def increment(
        ...
        *,
        ...
        with self._lock:
            cur = self.conn.execute(
                f"SELECT ctid AS _janus_ctid, * FROM {table_sql} "
    ...
    def query(
        ...
        sql = f"SELECT {select_cols} FROM ({inner_sql}) AS _janus_visible"
        if order_by:
    ...
    def delete(
        ...
        with self._lock:
            cur = self.conn.execute(
                f"SELECT ctid AS _janus_ctid, _end_txn FROM {table_sql} "
                f"WHERE {pk_sql} = %s AND {vis_clause} "
                f"ORDER BY _begin_txn DESC LIMIT 1",
                [pk_val] + vis_params,
            )
            ...
            self.conn.execute(
                f"UPDATE {table_sql} SET _end_txn = %s WHERE ctid = %s",
                [txn.numeric_id, existing["_janus_ctid"]],
            )
            self.conn.commit()
