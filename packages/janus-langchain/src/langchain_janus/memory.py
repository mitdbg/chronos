"""Janus Memory tool — persistent agent memory backed by overlay files.
...
MEMORY PROTOCOL:
1. Use the `list` command of your `janus_memory` tool to check for earlier progress.
...
class JanusMemoryInput(BaseModel):
    """Input schema for the Janus memory tool."""
...
class JanusMemory(BaseTool):
    ...
    Example::
        ...
        mem = JanusMemory(working_dir=Path("/tmp/tar_overlayfs/.../merged"))
    ...
    args_schema: type[BaseModel] = JanusMemoryInput
