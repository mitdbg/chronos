"""Shared test fixtures for Chronos-Code tests."""
...
class MockChronosContext:
    """Minimal ChronosContext-compatible shim for tests without real OverlayFS."""
    ...
    def begin(self):
        ...
        self._sqlite = MockChronosSQLite(shim=self._sqlite_shim, txn=self._txn)
        return self._txn
...
class MockSQLiteShim:
    """Minimal in-memory SQLiteShim-like store for Chronos-code unit tests."""
...
def memory_dir(tmp_workspace: Path) -> Path:
    """Create and return the .chronos-code/memory directory."""
    mem = tmp_workspace / ".chronos-code" / "memory"
    mem.mkdir(parents=True)
    return mem


@pytest.fixture
def chronos_context(tmp_workspace: Path):
    """ChronosContext for Chronos integration tests.

    Skips automatically if langchain_chronos is unavailable or if the system
    ...
    ctx = None
    try:
        ctx = ChronosContext(str(tmp_workspace), enable_sqlite=True, enable_vectorstore=False)
    except Exception as e:
        pytest.skip(f"ChronosContext unavailable: {e}")
    yield ctx
...
def patch_session_manager_default_tar(monkeypatch: pytest.MonkeyPatch):
    """Inject a mock ChronosContext when tests construct SessionManager without one."""
    from chronos_code.chronos_integration import session_manager as sm_mod

    original_init = sm_mod.SessionManager.__init__

    def patched_init(self, project_path, config, chronos_context=None, db_path=":memory:"):
        effective_chronos = chronos_context or MockChronosContext(Path(project_path))
        return original_init(
            self,
            project_path=project_path,
            config=config,
            chronos_context=effective_chronos,
            db_path=db_path,
        )
