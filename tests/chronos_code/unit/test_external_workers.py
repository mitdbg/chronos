...
from chronos_code.agent.external_workers import CommandResult, ExternalWorkerAdapter
...
async def test_external_worker_writes_mcp_config_and_prompt(tmp_path: Path) -> None:
    ...
    summary = await adapter.run_worker(
        ...
        task_contract={
            "node_id": "n1",
            "objective": "Implement feature X",
            "allowed_tools": ["chronos_txn", "chronos_file_editor"],
        },
    ...
    assert server["command"] == "chronos-code-mcp"
