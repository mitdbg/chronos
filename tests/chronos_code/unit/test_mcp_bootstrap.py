"""Unit tests for Chronos MCP bootstrap helpers."""
...
from chronos_code.mcp_server.bootstrap import bootstrap_project
...
def test_bootstrap_writes_mcp_and_instruction_assets(tmp_path: Path) -> None:
    ...
    tar_server = config["mcpServers"]["chronos"]
    assert tar_server["command"] == "chronos-code-mcp"
    assert "--project" in tar_server["args"]
    assert str(tmp_path.resolve()) in tar_server["args"]

    codex_instruction = tmp_path / ".chronos-code/bootstrap/codex/AGENTS.chronos.md"
    codex_skill = (
        tmp_path / ".chronos-code/bootstrap/codex/skills/chronos-transaction-protocol/SKILL.md"
    )
    claude_instruction = tmp_path / ".chronos-code/bootstrap/claude/CLAUDE.chronos.md"
    claude_script = tmp_path / ".chronos-code/bootstrap/claude/register_mcp.sh"
...
def test_bootstrap_keeps_existing_server_entry_without_overwrite(tmp_path: Path) -> None:
    ...
    mcp_path.write_text(
        json.dumps(
            {
                "mcpServers": {
                    "chronos": {"command": "existing-command", "args": ["--old"]},
    ...
    config = _load_json(mcp_path)
    assert config["mcpServers"]["chronos"]["command"] == "existing-command"
...
def test_bootstrap_overwrites_existing_entry_when_requested(tmp_path: Path) -> None:
    mcp_path = tmp_path / ".mcp.json"
    mcp_path.write_text(
        json.dumps({"mcpServers": {"chronos": {"command": "old-cmd"}}}, indent=2) + "\n",
        encoding="utf-8",
    )
    ...
    bootstrap_project(
        ...
        mcp_command="custom-chronos-mcp",
    )

    config = _load_json(mcp_path)
    assert config["mcpServers"]["chronos"]["command"] == "custom-chronos-mcp"
...
def test_bootstrap_http_transport_writes_url_entry(tmp_path: Path) -> None:
    ...
    assert config["mcpServers"]["chronos"]["url"] == "http://127.0.0.1:9876/mcp"
