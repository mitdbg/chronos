...
from chronos_code.chronos_integration.session_manager import TxnContext
...
class _FakeEmbeddedServer:
    ...
    def register_session(self, session_id: str, chronos_context: object) -> None:
        self.registered.append((session_id, chronos_context))
...
async def test_run_external_worker_uses_embedded_mcp_url(tmp_path: Path) -> None:
    ...
    chronos_ctx = SimpleNamespace(is_active=True)
    txn = TxnContext(txn_id="txn7", chronos_context=chronos_ctx)
    ...
    assert registered_ctx is chronos_ctx
