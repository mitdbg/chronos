...
These tests validate real Janus transactional guarantees used by coding/debugging
...
def _require_tar(janus_context):
    if janus_context is None:
        pytest.skip("JanusContext unavailable")
...
class TestLiveSpeculativeExecution:
    def test_speculative_debug_attempt_abort_then_commit(
        ...
        session = SessionManager(
            ...
            db_path=str(tmp_workspace / ".janus-code" / "speculative.db"),
        )
        ...
        b_workdir = janus_context.working_dir
...
class TestLiveConcurrencyIsolation:
    def test_two_concurrent_transactions_have_isolated_views(
        ...
        state_dir = tmp_workspace / ".janus-code"
    ...
    def test_weak_snapshot_toggles_shim_conflict_controls(
        ...
        strong_ctx = JanusContext(
            str(tmp_workspace),
            db_path=strong_db,
            enable_sqlite=True,
            enable_vectorstore=False,
        )
        weak_ctx = JanusContext(
            str(tmp_workspace),
            db_path=weak_db,
            enable_sqlite=True,
            enable_vectorstore=False,
