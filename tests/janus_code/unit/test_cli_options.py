...
from janus_code.cli import _create_janus_context, main
...
def test_create_janus_context_accepts_weak_snapshot_flag(tmp_path) -> None:
    """JanusContext factory should propagate weak snapshot configuration."""
    strong_ctx = _create_janus_context(tmp_path, weak_snapshot=False)
    weak_ctx = _create_janus_context(tmp_path, weak_snapshot=True)
