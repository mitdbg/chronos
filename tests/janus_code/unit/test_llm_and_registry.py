...
from janus_code.agent.llm import MockLLM, build_tool_schemas
from janus_code.tools.registry import build_tools, build_tool_schemas as registry_build_schemas
from janus_code.config import Config
from tests.janus_code.conftest import MockJanusContext
...
class TestBuildTools:
    ...
    def test_sqlite_tool_present_with_janus_context(self, tmp_path):
        config = Config()
        janus_context = MockJanusContext(tmp_path)
        janus_context.begin()
        tools = build_tools(working_dir=tmp_path, config=config, janus_context=janus_context)
