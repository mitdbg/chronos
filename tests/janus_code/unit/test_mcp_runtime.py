"""Unit tests for Janus MCP runtime session and transaction behavior."""
...
def _build_runtime(tmp_path: Path) -> JanusMcpRuntime:
    return JanusMcpRuntime(
        JanusMcpRuntimeConfig(project_dir=tmp_path),
        context_factory=lambda base_path, **kwargs: _FakeContext(base_path, **kwargs),
    )


def _build_enforced_runtime(tmp_path: Path, enforced_session_id: str) -> JanusMcpRuntime:
    return JanusMcpRuntime(
...
def test_tools_require_active_transaction(tmp_path: Path) -> None:
    ...
    output = runtime.janus_file_editor(
        command="view",
        path="README.md",
        session_id="agent-a",
    )
...
def test_transaction_lifecycle_status_and_savepoints(tmp_path: Path) -> None:
    ...
    start = runtime.janus_txn(action="begin", session_id=sid)
    assert "Transaction started" in start

    status_active = runtime.janus_txn(action="status", session_id=sid)
    assert "Transaction active" in status_active
    assert "session=agent-a" in status_active

    sp = runtime.janus_txn(action="savepoint", session_id=sid, name="before_refactor")
    assert "Savepoint 'before_refactor' created" in sp

    rb = runtime.janus_txn(action="rollback", session_id=sid, name="before_refactor")
    assert "Rolled back savepoint 'before_refactor'" in rb

    done = runtime.janus_txn(action="commit", session_id=sid)
    assert "Transaction committed" in done
    assert "No active transaction" in runtime.janus_txn(action="status", session_id=sid)


def test_sessions_are_isolated(tmp_path: Path) -> None:
    runtime = _build_runtime(tmp_path)

    runtime.janus_txn(action="begin", session_id="agent-a")
    runtime.janus_txn(action="begin", session_id="agent-b")

    out_a = runtime.janus_file_editor(
        command="create",
        path="a.txt",
        file_text="hello-a",
        session_id="agent-a",
    )
    out_b = runtime.janus_file_editor(
        command="create",
        path="b.txt",
        file_text="hello-b",
        session_id="agent-b",
...
def test_enforced_session_id_maps_default_and_rejects_others(tmp_path: Path) -> None:
    ...
    start = runtime.janus_txn(action="begin")  # default alias
    assert "session=branch-a" in start

    ok = runtime.janus_file_editor(
        command="create",
        path="x.txt",
        file_text="hello",
        session_id="default",
    ...
    denied = runtime.janus_bash(command="pwd", session_id="other-branch")
