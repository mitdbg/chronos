"""Tests for ChronosContext and ChronosTransactionControl.
...
def fresh_base_dir() -> Generator[Path, None, None]:
    ...
    _require_root()
    d = Path(tempfile.mkdtemp(prefix="chronos_ctx_test_"))
...
# ── ChronosContext lifecycle tests ───────────────────────────────────────


class TestChronosContextLifecycle:
    ...
    def test_begin_creates_overlay(self, fresh_base_dir: Path) -> None:
        ...
        ctx = ChronosContext(fresh_base_dir)
        txn = ctx.begin()
        ...
        # Tools are available
        assert isinstance(ctx.file_editor, ChronosFileEditor)
        assert isinstance(ctx.memory, ChronosMemory)
        assert isinstance(ctx.bash, ChronosBash)
...
class TestContextManager:
    ...
    def test_context_manager_auto_aborts(self, fresh_base_dir: Path) -> None:
        ...
        with ChronosContext(fresh_base_dir) as ctx:
            ctx.file_editor.invoke(
...
# ── ChronosTransactionControl tests ─────────────────────────────────────


class TestChronosTransactionControl:
...
# ── create_chronos_tools convenience function tests ─────────────────────


class TestCreateChronosTools:
    """Tests for create_chronos_tools and create_chronos_tools_with_active_txn."""

    def test_create_chronos_tools(self, fresh_base_dir: Path) -> None:
        """create_chronos_tools returns context and control tool."""
        ctx, tools = create_chronos_tools(fresh_base_dir)
        assert not ctx.is_active  # Not begun yet
        assert len(tools) == 1  # Only txn_control
        assert tools[0].name == "chronos_txn"

    def test_create_chronos_tools_with_active_txn(self, fresh_base_dir: Path) -> None:
        """create_chronos_tools_with_active_txn returns everything ready."""
        ctx, tools = create_chronos_tools_with_active_txn(fresh_base_dir)
        try:
            assert ctx.is_active
