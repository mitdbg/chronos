"""Subtransaction and branching tests for Chronos.
...
class TestSavepointDBIsolation:
    ...
    def test_rollback_discards_db_inserts(self, fresh_dir: Path) -> None:
        ...
        ctx = ChronosContext(fresh_dir, enable_sqlite=True)
        ctx.begin()
...
# ── ChronosTransactionControl savepoints ─────────────────────────────────


class TestTxnCtlSavepoints:
    """Savepoint/rollback via the ChronosTransactionControl tool."""

    def test_ctl_savepoint_and_rollback(self, fresh_dir: Path) -> None:
        """ChronosTransactionControl savepoint/rollback works end-to-end."""
        ctx = ChronosContext(fresh_dir, enable_sqlite=True)
