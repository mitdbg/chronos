...
class TestAtomicCommit:
    ...
    def test_commit_persists_fs_and_db(self, fresh_base: Path) -> None:
        ...
        ctx2 = JanusContext(fresh_base, enable_sqlite=True,
...
class TestGetToolsIncludesSQLite:
    ...
    def test_get_tools_without_sqlite(self, fresh_base: Path) -> None:
        ctx = JanusContext(fresh_base, enable_sqlite=False)
        ctx.begin()
        tools = ctx.get_tools()
        names = {t.name for t in tools}
        assert "janus_sqlite" not in names
        assert "janus_vectorstore" in names  # vectorstore is still enabled
        assert len(tools) == 4
        ctx.abort()

    def test_create_janus_tools_with_active_txn_includes_sqlite(
        self, fresh_base: Path
    ) -> None:
        ctx, tools = create_janus_tools_with_active_txn(fresh_base)
        try:
            names = {t.name for t in tools}
            assert "janus_sqlite" in names
            assert "janus_vectorstore" in names
...
# ── JanusTransactionControl with sqlite ─────────────────────────────


class TestTransactionControlWithSQLite:
    """The JanusTransactionControl tool manages both FS and DB."""
